...
public interface SniLogRepository extends JpaRepository<SniLog, Long> {
    ...
    @Query("SELECT s.sniHost, s.protocol, COUNT(s) FROM SniLog s GROUP BY s.sniHost, s.protocol ORDER BY COUNT(s) DESC")
    List<Object[]> findTopSites();

    List<SniLog> findByClientIpOrderByDetectTimeDesc(String clientIp);

    @Query("SELECT s.sniHost, s.protocol, COUNT(s) FROM SniLog s WHERE s.detectTime >= :since GROUP BY s.sniHost, s.protocol ORDER BY COUNT(s) DESC")
    List<Object[]> findTopSitesSince(LocalDateTime since);

    @Query("SELECT s.listenPort, s.sniHost, s.protocol, COUNT(s) FROM SniLog s WHERE s.detectTime >= :since GROUP BY s.listenPort, s.sniHost, s.protocol ORDER BY s.listenPort ASC, COUNT(s) DESC")
    ...
    @Query("SELECT s.sniHost, s.protocol, COUNT(s) FROM SniLog s WHERE s.clientIp = :ip GROUP BY s.sniHost, s.protocol ORDER BY COUNT(s) DESC")
    List<Object[]> findTopDomainsByIp(@Param("ip") String ip);

    @Query("SELECT s.sniHost, s.protocol, COUNT(s) FROM SniLog s WHERE s.listenPort = :listenPort AND s.clientIp = :ip GROUP BY s.sniHost, s.protocol ORDER BY COUNT(s) DESC")
